...
*    {@link #queryNorm(clucene_float_t) queryNorm}(q)
*    </td>
*  </tr>
*  <tr>
...
public:
   ...
   */
   static uint8_t encodeNorm(clucene_float_t f);
   
   /** Decodes a normalization factor stored in an index.
   * @see #encodeNorm(clucene_float_t)
   */
   static clucene_float_t decodeNorm(uint8_t b);
   
   static uint8_t floatToByte(clucene_float_t f);
   static clucene_float_t byteToFloat(uint8_t b);
   ...
   */
   clucene_float_t idf(CL_NS(util)::CLVector<CL_NS(index)::Term*>* terms, Searcher* searcher);

   template<class TermIterator>
   clucene_float_t idf( TermIterator first, TermIterator last, Searcher* searcher )
   {
      clucene_float_t _idf = 0.0f;
   ...
   //clucene_float_t idf(Term** terms, Searcher* searcher);
   ...
   */
   clucene_float_t idf(CL_NS(index)::Term* term, Searcher* searcher);
   ...
   * is small.
   *
   * <p>The default implementation calls {@link #tf(clucene_float_t)}.
   *
   * @param freq the frequency of a term within a document
   * @return a score factor based on a term's within-document frequency
   */
   inline clucene_float_t tf(int32_t freq){ return tf((clucene_float_t)freq); }
   ...
   * {#encodeNorm(clucene_float_t)}.  Thus they have limited precision, and documents
   ...
   * @see Field#setBoost(clucene_float_t)
   */
   virtual clucene_float_t lengthNorm(const TCHAR* fieldName, int32_t numTokens) = 0;
   ...
   */
   virtual clucene_float_t queryNorm(clucene_float_t sumOfSquaredWeights) = 0;

   /** Computes the amount of a sloppy phrase match, based on an edit distance.
   * This value is summed for each sloppy phrase match in a document to form
   * the frequency that is passed to {@link #tf(clucene_float_t)}.
   *
   ...
   */
   virtual clucene_float_t sloppyFreq(int32_t distance) = 0;
   ...
   * @return a score factor based on a term's within-document frequency
   */
   virtual clucene_float_t tf(clucene_float_t freq) = 0;
   ...
   */
   virtual clucene_float_t idf(int32_t docFreq, int32_t numDocs) = 0;
   ...
   */
   virtual clucene_float_t coord(int32_t overlap, int32_t maxOverlap) = 0;
};
...
public:
  ...
  clucene_float_t lengthNorm(const TCHAR* fieldName, int32_t numTerms);
  
  /** Implemented as <code>1/sqrt(sumOfSquaredWeights)</code>. */
  clucene_float_t queryNorm(clucene_float_t sumOfSquaredWeights);

  /** Implemented as <code>sqrt(freq)</code>. */
  inline clucene_float_t tf(clucene_float_t freq);
    
  /** Implemented as <code>1 / (distance + 1)</code>. */
  clucene_float_t sloppyFreq(int32_t distance);
    
  /** Implemented as <code>log(numDocs/(docFreq+1)) + 1</code>. */
  clucene_float_t idf(int32_t docFreq, int32_t numDocs);
    
  /** Implemented as <code>overlap / maxOverlap</code>. */
  clucene_float_t coord(int32_t overlap, int32_t maxOverlap);
};
