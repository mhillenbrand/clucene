...
FieldDoc::FieldDoc (int32_t doc, clucene_float_t score)
{
...
FieldDoc::FieldDoc (int32_t doc, clucene_float_t score, CL_NS(util)::Comparable** fields)
{
	this->scoreDoc.doc = doc;
	this->scoreDoc.score = score;
	this->fields = fields;
		...
		if (fields[i]->getReverse()) {
			switch (type) {
				...
				case SortField::CUSTOM:
					...
					break;
				case SortField::AUTO:
					// we cannot handle this - even if we determine the type of object (clucene_float_t or
					// Integer), we don't necessarily know how to compare them (both SCORE and
					// clucene_float_t both contain floats, but are sorted opposite of each other). Before
