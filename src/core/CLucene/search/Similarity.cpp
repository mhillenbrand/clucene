...
CL_NS_DEF(search)
	...
	clucene_float_t Similarity::byteToFloat(uint8_t b) {
		return NORM_TABLE[b];
    }

   uint8_t Similarity::floatToByte(clucene_float_t f) {
	...
	clucene_float_t NORM_TABLE[256];
	...
	int32_t floatToIntBits(clucene_float_t value)
	{
		clvalue u;
		int32_t e, f;
		u.f = (float)value;
	...
	clucene_float_t intBitsToFloat(int32_t bits)
	{
		clvalue u;
		u.i = bits;
		return u.f;
   ...
   uint8_t Similarity::floatToByte(clucene_float_t f) {
      ...
      int32_t bits = floatToIntBits(f);           // parse clucene_float_t into parts
   ...
   clucene_float_t Similarity::decodeNorm(uint8_t b) {
   ...
   uint8_t Similarity::encodeNorm(clucene_float_t f) {
#ifdef _CL_HAVE_NO_FLOAT_BYTE
	   int32_t i=0;
	   if ( f <= 0 )
		   return 0;
   ...
   clucene_float_t Similarity::idf(Term* term, Searcher* searcher) {
      return idf(searcher->docFreq(term), searcher->maxDoc());
   }


   clucene_float_t Similarity::idf(CL_NS(util)::CLVector<Term*>* terms, Searcher* searcher) {
  ...
  clucene_float_t DefaultSimilarity::lengthNorm(const TCHAR* /*fieldName*/, int32_t numTerms) {
    if ( numTerms == 0 ) //prevent div by zero
        return 0;
    return (1.0 / sqrt((clucene_float_t)numTerms));
  }

  clucene_float_t DefaultSimilarity::queryNorm(clucene_float_t sumOfSquaredWeights) {
    if ( sumOfSquaredWeights == 0 ) //prevent div by zero
        return 0.0f;
	  return (clucene_float_t)(1.0 / sqrt(sumOfSquaredWeights));
  }

  clucene_float_t DefaultSimilarity::tf(clucene_float_t freq) {
    return sqrt(freq);
  }

  clucene_float_t DefaultSimilarity::sloppyFreq(int32_t distance) {
    return 1.0f / (distance + 1);
  }

  clucene_float_t DefaultSimilarity::idf(int32_t docFreq, int32_t numDocs) {
    return (clucene_float_t)(log(numDocs/(clucene_float_t)(docFreq+1)) + 1.0);
  }

  clucene_float_t DefaultSimilarity::coord(int32_t overlap, int32_t maxOverlap) {
  	if ( maxOverlap == 0 )
  		return 0.0f;
    return overlap / (clucene_float_t)maxOverlap;
  }
CL_NS_END
