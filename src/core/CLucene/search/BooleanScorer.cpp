...
CL_NS_DEF(search)
  ...
  bool BooleanScorer::next() {
	...
	clucene_float_t BooleanScorer::score(){
  ...
  void BooleanScorer::computeCoordFactors(){
    coordFactors = _CL_NEWARRAY(clucene_float_t,maxCoord);
  ...
  void BooleanScorer::Collector::collect(const int32_t doc, const clucene_float_t score){
